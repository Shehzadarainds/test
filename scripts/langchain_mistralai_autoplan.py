...
def load_langchain_config(config_path='../agent/config_Mistral.json'):
    ...
    llm_kwargs = {
        'model': 'mistral',  # Use Ollama Mistral model
        'base_url': 'https://returned-eric-wedding-ram.trycloudflare.com/',  # Custom Ollama URL
    ...
    for server_name, server_config in config['mcpServers'].items():
        ...
        for arg in server_config['args']:
            if 'tmp/tmp/out' in arg:
                ...
                # Tools are in ../agent/tools/ relative to scripts directory
        ...
        mcp_servers[server_name] = {
            ...
            "transport": "stdio",
            "env": {
                "PYTHONPATH": str(Path(__file__).parent.parent / 'agent' / 'tools')
            }
        }
...
async def create_langchain_agent(llm, mcp_servers):
    ...
    try:
        ...
        # Get tools from all MCP servers with better error handling
        try:
            tools = await client.get_tools()
            print(f"Successfully loaded {len(tools)} tools from MCP servers")
        except Exception as mcp_error:
            print(f"\nMCP Server Initialization Error: {mcp_error}")
            print("\nTroubleshooting information:")
            print(f"1. Check if Python tool files exist in ../agent/tools/")
            print(f"2. Verify PYTHONPATH includes the tools directory")
            print(f"3. Ensure all tool dependencies are installed")
            print(f"4. Check if the MCP servers are properly implemented")
            ...
            if tools_dir.exists():
                ...
                print(f"Tool files found: {[f.name for f in tools_dir.glob('*.py')]}")
            else:
...
def load_questions(test_json_path: str = 'benchmark/question.json'):
    ...
    # Handle relative path - go up one directory from scripts folder
    if not Path(test_json_path).is_absolute():
        test_json_path = str(Path(__file__).parent.parent / test_json_path)
...
async def main():
    ...
    try:
        # Find which questions have available data
        data_dir = Path(__file__).parent.parent / 'benchmark' / 'data'
        available_questions = set()
        
        if data_dir.exists():
            for qdir in data_dir.glob('question*'):
                if qdir.is_dir() and list(qdir.glob('*')):  # Has files
                    try:
                        question_num = int(qdir.name.replace('question', ''))
                        available_questions.add(str(question_num))
                    except ValueError:
                        pass
        
        print(f"\n✓ Found {len(available_questions)} questions with available data")
        if available_questions:
            print(f"  Questions: {sorted([int(q) for q in available_questions])}")
        
        # Load all questions
        all_questions = load_questions()
        print(f"Loaded {len(all_questions)} total questions from benchmark")
        
        # Filter to only available questions
        questions = [q for q in all_questions if q['question_id'] in available_questions]
        
        if not questions:
            print("\n✗ No questions with available data found!")
            return
        
        print(f"Processing {len(questions)} available questions for evaluation")
